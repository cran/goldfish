...
* **isTwoMode**: Identifies whether the effect assumes the network is originated
...
the DyNAM-choice submodel only considers change statistics for potential receivers
...
Homophily (same value). The tendency of an event $i\rightarrow j$ to happen if actors i and j have the same attribute value.
This effect cannot be used for two-mode networks and for the DyNAM-rate submodel.
...
Heterophily. The tendency of an event $i\rightarrow j$ to happen if actors i and j 
...
\begin{equation}
s(i,j, t, x, z) = \left|\frac{z(t)_i - \sum_{k:~x(t)_{kj} > 0}{z(t)_k}}{ \sum_{k}{I(x(t)_{kj} > 0)}}\right|
\end{equation}
