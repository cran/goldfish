...
test_that("diff returns correct attributes on update", {
  ...
  expect_equal(update_DyNAM_choice_diff(testAttr$fishingSkill,
    node = 1, replace = 0,
    n1 = 8, n2 = 0
  )$changes,
  rbind(
    cbind(node1 = rep(1, 7), node2 = 2:8, replace = testAttr$fishingSkill[-1]),
    cbind(node1 = 2:8, node2 = rep(1, 7), replace = testAttr$fishingSkill[-1])
  ),
  ...
  rbind(
    ...
    cbind(node1 = c(1, 3:8), node2 = rep(2, 7),
          replace = c(0, 5, 0, 2, 2, 7, NA))
  ),
